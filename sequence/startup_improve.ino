...
//const task tasklist[4]={{GREEN[1],0},{GREEN[2],1},{BLUE[1],0},{BLUE[2],1}};
...
//*****************arm
void armrelease(int grab_angle, bool DoRelease){
  if(!DoRelease) return;
...
void rotate_cw_90(){
  for (int i=0;i<2106;i++){ //2110 is a changeable parameter
    rotate_cw_step();
    delayMicroseconds(1000); // 2000 depends on the frequency of timer interrupt
  }
}
...
//先左右走后前后走可能可以让停车的时候姿态更准一点
  ...
  switch (state){
    ...
    case run_line_lr:{//左右移动，巡双线，判定单线，走n格的话检测到第n次黑线立刻停车（一开始理论上为黑黑黑）
      ...
      if(delta_x>0 && grid_counter==delta_x || delta_x<0 && grid_counter==-(delta_x)) {state++;grid_counter=0;}//现在规避了第一根线，所以不加一
      break;
    }
    ...
    case run_line_fb:{//前后移动，巡单线，判定双线，走n格的话检测到2n次黑线退出后停车（一开始理论上为白黑白）
      ...
      if(delta_y>0 && grid_counter==2*delta_y-1 || delta_y<0 && grid_counter==-2*delta_y-1){
   ...
   case startup:{
      if(p_timerM<timerM) startup_counter++;
      if(direction_key==1) {forward_step();Correction();}
      if(direction_key==-1) {backward_step();Correction();}
      if(direction_key==2) {left_step();Correction();}
      if(direction_key==-2) {right_step();Correction();}
      if(startup_counter>1500){
        if(direction_key==1 || direction_key==-1) state=run_line_fb;
        if(direction_key==2 || direction_key==-2) state=run_line_lr;
        startup_counter=0;
        break;
      }
   }
...
//  int sum=distace/s;//圈数
  ...
  if(curr_task.target.y==5){
    ...
    //delay(1000);//temp
    delay(500);//temp,用delay来表示正在执行抓取
    //rotate_cw_180();
    if(curr_task.op==0)
    hold(high_grab,1);
    if(curr_task.op==1)
    armrelease(low_grab,1);
    //rotate_cw_180();
    delay(500); //在delay中定时器中断在不在计时？
